...
from member.models import AuthToken, Profile, CompletedTaskCount, Event , PushNotification
...
class ContentView(APIView):  
    ...
    def post(self, request):
        ...
        if is_portion == 'yes':
            ...
            print("pppppppppppp:", session)
            print("gggggg:", session.image)
            print("gggggg:", session.image.url)
            print("gggggg:", session.video)

            if session.image == "" or session.image == None:
                simg = ""
            else:
                simg = session.image.url

            if session.video == "" or session.video == None:
                svideo = ""
            else:
                svideo = session.video.url

            if session.audio == "" or session.audio == None:
                saudio = ""
            else:
                saudio = session.audio.url
    ...
    def post(self, request):
        ...
        if is_portion == "yes":
            ...
            if created:
                ...
                for obj in fav_panel.session_segments.all():
                    if obj.id == part.id:
                        ...
                        message = f"User unliked session part: {part.title}"
                    else:
                        fav_panel.session_segments.add(part)
                        message = f"User liked session part: {part.title}"
        else:
            fav_panel, created = FavoriteSession.objects.get_or_create(user=user_obj)
            session = Session.objects.filter(id=session_id).first()
            ...
            if created:
                ...
                message = f"User liked session: {session.title}"
            else:
                if session_id in p_list:
                    fav_panel.sessions.remove(session)
                    message = f"User unliked session: {session.title}"
                else:
                    fav_panel.sessions.add(session)
                    message = f"User liked session: {session.title}"
...
class FavoritePsychiatristsView(APIView):    
    def post(self, request):
        ...
        if created:
            message = "User added a psychiatrist to favorites"
        else:
            message = "User removed a psychiatrist from favorites"
